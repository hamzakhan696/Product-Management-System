...
import TenantTable from "../../components/tables/tenant/tenantdetailtable";
...
import TenantEditModal from "./tenantmodal/edittenantmodel";
...
function TenantDetail() {
  ...
  useEffect(() => {
    getData();
    if (selection) {
      localStorage.setItem("Tenantid", selection);
    }
  }, [selection, data]);

  const deleteData = async () => {
    try {
      await axios.delete(`${BASE_URL}${apiRoutes.deleteTenants}`, {
        data: { ids: selection },
      });
            ...
            {selection.length > 0 ? (
              <>
                <CusButton
                  ...
                  onClick={deleteData}
                />
                <TenantEditModal disabled={selection.length > 1} />
                <CusButton
                  variant="default"
                  type="submit"
                  style={{
              ...
              <>
                ...
                >
                  All Tenant Details <span style={{ color: "red" }}>*</span>
                </Text>
              </>
            )}
            <Group
