...
import AmenitiesTable from "../../components/tables/amenities/amenitiesdetailtable";
...
import AmenitiesModal from "./amenitiesModel/amenitiesModal";
import { AmenitiesEditModal } from "./amenitiesModel/editamenities";
...
function AmenitiesDetail() {
  ...
  console.log(selection, "Amenties");
  const getData = async () => {
    try {
      const res = await axios.get(`${BASE_URL}${apiRoutes.getAmenities}`, {
      ...
      >
        <Flex justify="space-between" direction="row" wrap="wrap">
          {selection.length > 0 ? (
            ...
            >
              All Amenities Details <span style={{ color: "red" }}>*</span>
            </Text>
          )}
          <Group
            spacing="lg"
