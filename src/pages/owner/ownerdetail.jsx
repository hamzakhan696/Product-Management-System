...
import classes from "./ownerdetail.module.css";
import OwnerTable from "../../components/tables/owner/ownerdetailtable";
...
function OwnerDetail() {
  ...
  const getData = async () => {
    try {
      const res = await axios.get(`${BASE_URL}${apiRoutes.allOwner}`);
      setData(res.data);
    } catch (error) {
        ...
        <Flex justify="space-between" direction="row" wrap="wrap">
          {selection.length > 0 ? (
            ...
            <>
              ...
              >
                All Owner Details <span style={{ color: "red" }}>*</span>
              </Text>
            </>
          )}
          <Group
