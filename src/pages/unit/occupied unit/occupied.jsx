...
import { OccupiedEditModal } from "./editoccupied";
import { UnitAddModal } from "../addunit";
import { useNavigate } from "react-router-dom";

function OccupiedDetail() {
  ...
  const getData = async () => {
    try {
      const res = await axios.get(`${BASE_URL}${apiRoutes.occupiedunit}`);
      setData(res.data);
        ...
        <Flex justify="space-between" direction="row" wrap="wrap">
          {selection.length > 0 ? (
            ...
            <>
              ...
              >
                All Occupied Details <span style={{ color: "red" }}>*</span>
              </Text>
            </>
          )}
          <Group
